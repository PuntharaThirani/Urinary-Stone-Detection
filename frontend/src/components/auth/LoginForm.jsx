...
const imageMap = {
  patient: 'https://images.unsplash.com/photo-1516574187841-cb9cc2ca948b?auto=format&fit=crop&q=80&w=800',
  doctor:  'https://images.unsplash.com/photo-1622253692010-333f2da6031d?q=80&w=1000&auto=format&fit=crop',
  staff:   'https://images.unsplash.com/photo-1516549655169-df83a0774514?auto=format&fit=crop&q=80&w=1000',
  admin:   'https://images.unsplash.com/photo-1551434678-e076c223a692?auto=format&fit=crop&q=80&w=1000',
};
...
// ✅ All paths consistent /xxx-dashboard pattern
const dashboardMap = {
  doctor:  '/doctor-dashboard',
  patient: '/patient-dashboard',
  staff:   '/staff-dashboard',
  admin:   '/admin-dashboard',  // ✅ Fix: /admin → /admin-dashboard
};
...
const LoginForm = ({ adminMode = false }) => {
  ...
  useEffect(() => {
    if (adminMode) { setUserType('admin'); return; }
    const queryParams = new URLSearchParams(location.search);
    const mode = queryParams.get('mode');
    const role = queryParams.get('role');
    setIsLogin(mode !== 'register');
    if (role && roles.includes(role)) setUserType(role);
    else setUserType('doctor');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const id    = response?.user?.id || response?.user?._id || ''; // ✅ _id fallback
        ...
        <div className="relative hidden w-1/2 overflow-hidden md:block">
          ...
          <div className="absolute bottom-10 left-8 right-8 z-20">
            <div className="max-w-md rounded-3xl border border-white/20 bg-white/10 p-6 text-white backdrop-blur-md">
              ...
              <h2 className="mt-3 text-3xl font-bold leading-tight">AI-Assisted Urinary Stone Detection</h2>
              <p className="mt-4 text-sm leading-6 text-slate-100/90">
                Access the platform to upload X-ray images, review detection results, and support report preparation through a structured workflow.
              </p>
              <div className="mt-6 inline-flex items-center gap-3 rounded-2xl bg-white px-4 py-3 text-slate-800 shadow-lg">
                <span className="text-xl">✓</span>
                <div>
                  <p className="text-[10px] font-extrabold uppercase tracking-[0.22em] text-slate-400">System Status</p>
        ...
        <div className="flex w-full items-center justify-center bg-white px-6 py-10 md:w-1/2 md:px-12 lg:px-16">
          <div className="w-full max-w-md">
            ...
            <p className="mt-3 text-base leading-7 text-slate-500">
              {isLogin ? 'Sign in to continue to your system portal.' : 'Create your patient account to access the platform.'}
            </p>
            ...
            {isLogin ? (
              <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
                ...
                <div className="space-y-2">
                  <label className="ml-1 block text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">Email Address</label>
                  <input
                ...
                <div className="space-y-2">
                  <label className="ml-1 block text-[11px] font-extrabold uppercase tracking-[0.18em] text-slate-500">Password</label>
                  <input
              ...
              <div className="mt-8">
                <RegisterForm defaultRole="patient" onSuccess={() => setIsLogin(true)} />
              </div>
            )}
            ...
            <div className="mt-8 rounded-2xl bg-slate-50 px-5 py-4">
              <p className="text-xs leading-6 text-slate-500">
                This system is intended to support medical workflow activities such as X-ray upload, AI-assisted result review, and report preparation.
              </p>
            </div>
